CREATE TABLE part (
    part_id INT AUTO_INCREMENT PRIMARY KEY,         -- Auto incrementing part_id (Primary Key)
    part_name VARCHAR(255) NOT NULL,                 -- part_name with a max length of 255 characters (NOT NULL)
    ...
    dimensions VARCHAR(30),                          -- dimensions with a max length of 30 characters
    part_weight DECIMAL(5, 2),                       -- part_weight as DECIMAL with 5 digits, 2 of which are after the decimal point
    part_type VARCHAR(100) NOT NULL,                 -- part_type with a max length of 100 characters (NOT NULL)
    part_category VARCHAR(100) NOT NULL              -- part_category with a max length of 100 characters (NOT NULL)
);

CREATE TABLE car (
    make VARCHAR(50) NOT NULL,                       -- make of the car (NOT NULL)
    model VARCHAR(50) NOT NULL,                      -- model of the car (NOT NULL)
...
CREATE TABLE person (
    email VARCHAR(50) NOT NULL,                      -- Email of the person (NOT NULL)
    p_password VARCHAR(256) NOT NULL,                -- Password of the person (NOT NULL)
    fname VARCHAR(50) NOT NULL,                      -- First name (NOT NULL)
    lname VARCHAR(50) NOT NULL,                      -- Last name (NOT NULL)
...
CREATE TABLE seller (
    seller_email VARCHAR(50) NOT NULL,               -- Seller's email (foreign key from person table)
    seller_org_name VARCHAR(255) NOT NULL,           -- Seller's organization name
...
CREATE TABLE buyer (
    buyer_email VARCHAR(50) NOT NULL,                -- Buyer's email (foreign key from person table)
...
CREATE TABLE shipment (
    shipment_id INT AUTO_INCREMENT PRIMARY KEY,     -- Auto-incrementing shipment ID
    shipment_date DATETIME NOT NULL,                 -- Shipment date
    shipment_address VARCHAR(255) NOT NULL,          -- Shipment address
    shipment_status VARCHAR(50) NOT NULL,            -- Shipment status
    ...
    seller_email VARCHAR(50) NOT NULL,               -- Seller email (foreign key from Seller)
    buyer_email VARCHAR(50) NOT NULL,                -- Buyer email (foreign key from Buyer)
...
CREATE TABLE part_sold_by (
    seller_email VARCHAR(50) NOT NULL,               -- Seller email (part of primary key)
...
CREATE TABLE payment_info (
    method_no INT AUTO_INCREMENT PRIMARY KEY,        -- Payment method number, auto-incremented, primary key
    buyer_email VARCHAR(50) NOT NULL,                 -- Buyer email, foreign key reference
    card_no VARCHAR(16) NOT NULL,                     -- Credit card number, 16 characters
    card_exp_date DATETIME NOT NULL,                  -- Card expiration date
    card_cvv INT NOT NULL,                            -- Card CVV code
    card_name VARCHAR(50) NOT NULL,                   -- Name on the card
    card_address VARCHAR(255) NOT NULL,               -- Card billing address
    FOREIGN KEY (buyer_email) REFERENCES buyer(buyer_email) -- Foreign key reference to Buyer
);

CREATE TABLE buyer_address (
    buyer_email VARCHAR(50) NOT NULL,                 -- Buyer email, foreign key reference
    line1 VARCHAR(255) NOT NULL,                      -- Address line 1, part of the primary key
    line2 VARCHAR(255),                               -- Address line 2, optional
    city VARCHAR(50) NOT NULL,                        -- City
    state_in VARCHAR(50) NOT NULL,                    -- State
...
CREATE TABLE part_image (
    path VARCHAR(255) NOT NULL,                       -- Path of the image, part of the composite primary key
    part_id INT NOT NULL,                             -- Part ID, foreign key reference to Part table
    image_name VARCHAR(255) NOT NULL,                  -- Name of the image
    description VARCHAR(255),                         -- Description of the image
